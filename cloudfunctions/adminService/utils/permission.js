const ADMIN_ROLES = ['systemAdmin', 'superAdmin', 'academyManager', 'approvalManager', 'custom']
...
function getRoleFromTags(permissionTags) {
  ...
  const hasAnyPermission = permissionTags.some(tag => {
    if (tag.permissions && typeof tag.permissions === 'object') {
      return Object.values(tag.permissions).some(v => v === true)
    }
    return false
  })
  if (hasAnyPermission) return 'custom'
  return null
}
